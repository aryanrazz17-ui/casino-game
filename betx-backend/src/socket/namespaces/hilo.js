...
module.exports = (hilo) => {
    ...
    HiloMService.init(hilo);

    hilo.on('connection', (socket) => {
        ...
        socket.on('place-bet', async (payload) => {
            try {
                ...
                if (success) {
                    ...
                    hilo.emit('bet', { userId: user.username, amount, status: 'BET' });

                    emitWalletUpdate(hilo, user.id, {
                        currency,
                        newBalance: updatedWallet.balance,
                        type: 'bet',
                        amount
                    });
                    emitHistoryUpdate(hilo, user.id);
                } else {
